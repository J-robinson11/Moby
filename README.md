...
| `WINDOW_HOURS` | `18` (per-sport) | Outer reach of a run's slate. Games before the **next scheduled run** are prioritized (last chance — their only shot); games after it are fallback only. Set per sport in each profile's `defaults` so each wakes on its own rhythm — daily sports (WNBA/NBA/MLB) at 18h, soccer 48h (knockout rest days), NFL 48h (preview Sunday from Saturday), UFC 96h (fight week). Env still overrides all |
